...
GCSStorage.prototype._handleFile = function (req, file, cb) {
  collect(this, req, file, function (err, opts) {
    ...
    var type = _mimeTypes["default"].lookup(file.originalname);
    ...
    var bucket = this.gcsClient.bucket(opts.bucket);
    ...
    if (opts.shouldCreateThumbnail) {
      ...
      //         cb(err);
      //     });
      var thumbBlob = bucket.file((0, _parseFilekey.parseFileKey)(opts.key, 'thumb'));
      var thumbFileStream = thumbBlob.createWriteStream({
        resumable: true,
        contentType: type,
        predefinedAcl: 'publicRead'
      });
      thumbFileStream.on('error', function (err) {
        cb(err);
      });
      thumbFileStream.on('finish', function () {
        cb(null, {
          size: thumbFileStream.bytesWritten,
          filename: (0, _parseFilekey.parseFileKey)(opts.key, 'thumb'),
          destination: "https://storage.googleapis.com/".concat(bucket.name, "/").concat(thumbBlob.name),
          url: "https://storage.googleapis.com/".concat(bucket.name, "/").concat(thumbBlob.name)
        });
    ...
    if (opts.shouldCreateFeatured) {
      ...
      //         cb(err);
      //     });
      var featuredBlob = bucket.file((0, _parseFilekey.parseFileKey)(opts.key, 'featured'));
      var featuredFileStream = featuredBlob.createWriteStream({
        resumable: true,
        contentType: type,
        predefinedAcl: 'publicRead'
      });
      featuredFileStream.on('error', function (err) {
        cb(err);
      });
      featuredFileStream.on('finish', function () {
        cb(null, {
          size: featuredFileStream.bytesWritten,
          filename: (0, _parseFilekey.parseFileKey)(opts.key, 'featured'),
          destination: "https://storage.googleapis.com/".concat(bucket.name, "/").concat(featuredBlob.name),
          url: "https://storage.googleapis.com/".concat(bucket.name, "/").concat(featuredBlob.name)
        });
    ...
    var primaryBlob = bucket.file(opts.key);
    var primaryFileStream = primaryBlob.createWriteStream({
      resumable: true,
      contentType: type,
      predefinedAcl: 'publicRead'
    });
    primaryFileStream.on('error', function (err) {
      cb(err);
    });
    primaryFileStream.on('finish', function () {
      cb(null, {
        size: primaryFileStream.bytesWritten,
        filename: opts.key,
        destination: "https://storage.googleapis.com/".concat(bucket.name, "/").concat(primaryBlob.name),
        url: "https://storage.googleapis.com/".concat(bucket.name, "/").concat(primaryBlob.name)
      });
...
GCSStorage.prototype._removeFile = function (req, file, cb) {
  ...
  this.gcsClient.bucket(file.bucket).file((0, _parseFilekey.parseFileKey)(file.key, '-thumb'))["delete"]();
  this.gcsClient.bucket(file.bucket).file((0, _parseFilekey.parseFileKey)(file.key, '-featured'))["delete"]();
  cb({});
};
